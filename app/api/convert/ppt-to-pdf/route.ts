...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await fetch(`${pythonEndpoint}/api/convert/ppt-to-pdf`, {
      method: "POST",
      body: pythonFormData,
    });
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `PPT to PDF conversion failed: ${errorText}` },
    ...
    return NextResponse.json(
      { error: error.message || "Failed to process PPT to PDF conversion" },
      { status: 500 }
    );
  }
}
